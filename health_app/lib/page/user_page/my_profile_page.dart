  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (user != null) {
        ...
        final stats = await _profileService.getUserStats(user.uid);
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildStatCard(
                  'Total Distance',
                  '${(_userStats!['totalDistance'] ?? 0).toStringAsFixed(1)} km',
          ...
          Row(
            children: [
              Expanded(
                child: _buildStatCard(
                  'Total Duration',
                  '${(_userStats!['totalDuration'] ?? 0).toStringAsFixed(0)} min',
                  Icons.timer,
                  Colors.orange,
                ),
              ),
              ...
              Expanded(
                child: _buildStatCard(
                  'Avg Speed',
                  '${(_userStats!['averageSpeed'] ?? 0).toStringAsFixed(1)} km/h',
                  Icons.speed,
                  Colors.purple,
                ),
              ),
